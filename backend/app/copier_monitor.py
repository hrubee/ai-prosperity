...
app = FastAPI(title="Tradejini Logs Monitor & Copier")

# Set up static files for the dashboard
...
def get_logs():
    ...
    try:
        ...
        pass
    # Logs are normally prepended in UI, we can just return chronological and let UI reverse
    return logs
...
# Global background task to run XTS websockets
xts_task = None

@app.on_event("startup")
async def startup_event():
    global xts_task
    # Start XTS websockets for all accounts
    xts_task = asyncio.create_task(start_xts_websockets(manager.broadcast))
...
async def receive_webhook(request: Request):
    ...
    # 1. Log the incoming signal immediately
    log_msg = {
    ...
    # Detect if this is a DhanHQ Postback or standard Copier Signal
    ...
    if is_dhan:
        ...
        if order_status in ("REJECTED", "CANCELLED", "FAILED", "EXPIRED"):
            ...
            mapping = get_order_mapping(dhan_order_id)
            if mapping:
                # Already mirrored as pending, do not place market order again
                return {"status": "ignored", "reason": "already placed as pending"}
            do_place = True
            tj_order_type = "market"  # Force a market order to guarantee fill
        else:
        ...
        # Normalize Dhan symbol (e.g., BANKNIFTY-Aug2026-57000-PE -> BANKNIFTY26AUG57000PE)
        ...
        # {"action": "BUY", "exchange": "NSECM", "symbol": "RELIANCE", "instrument_id": 2885, "quantity": 1}
    ...
    # 2. Replicate across all Tradejini accounts
    try:
        ...
        with db.session_scope() as session:
            ...
            for c in conns:
                if not tradejini_auth.has_auto_creds(c) or c.status == "disconnected":
                    continue
                try:
...
@app.get("/")
async def get():
    return RedirectResponse(url="/admin/copier")
