#!/usr/bin/env python3
"""backtest_official_stream_fibvol.py — Direct Engine Backtest of shared_scripts/stream_fibvol_coindcx.py.

Backtests the exact production strategy engine from shared_scripts/stream_fibvol_coindcx.py across 30 days of 1m intrabar OHLCV data.

Strategy Parameters:
- Entry Fib Level: 0.50 (High - 0.50 * Range)
- Stop Loss Fib Level: 0.60 (High - 0.60 * Range)
- Risk Reward Ratio: 1:5 RR (Entry + 5.0 * Risk)
- Multi-Candle Retracement Loop:
    * Re-plots Fib levels on subsequent green 15m candles
    * Cancels watch loop if subsequent 15m candle closes red
- Trailing SL: Activates at +2.0R profit, trailing 1.0R behind peak high
"""
import os
import sys
import json
...
def fetch_1m_klines(symbol):
    """Fetch 30 days of 1m OHLCV data."""
    try:
        sym = f"{symbol}USDT"
        all_rows = []
        endTime = None
...
def run_strategy_backtest(data_map, vol_spike_mult=30.0):
    """Replicates stream_fibvol_coindcx.py main execution loop minute-by-minute across all coins."""
    wallet_usd = START_BAL_USD
    equity_curve = [wallet_usd]
    
    # State tracking
    watching = {}  # base -> watch_dict
    positions = {} # base -> pos_dict
    trades_log = []

    # Get sorted timestamps for global simulation
    # To run multi-coin simulation properly, we aggregate 15m steps
    for sym, klines_1m in data_map.items():
        ...
        while i < n_15m - 2:
            ...
            # Signal Trigger: green candle + volume spike
            if spike_mult >= vol_spike_mult and is_green:
                watch_idx = i
                
                # Multi-candle watch loop (re-plotting on green, canceling on red)
                while watch_idx < n_15m - 1:
                    ...
                    # Check red candle cancellation rule
                    ...
                    # Compute Fib levels from stream_fibvol_coindcx.py
                    ...
                    # Simulate limit order fill on next 15m bar (1m granularity)
                    ...
                    if filled and fill_m1_idx >= 0:
                        # Order Filled! Simulate trade lifecycle with trailing SL
                        current_sl = sl_px
                        initial_sl = sl_px
                        peak_px = entry_px
                        outcome = None
                        ...
                        if outcome:
                            ...
                            wallet_usd += pnl_usd
                            equity_curve.append(wallet_usd)
...
def main():
    print("==========================================================================================")
    print("🔬 BACKTEST OF ACTUAL STRATEGY FILE: shared_strategies/open/fibvol.py & stream_fibvol_coindcx.py")
    print("   Parameters: Entry @ 0.50 Fib | Stop Loss @ 0.60 Fib | 1:5 Risk-Reward Ratio")
    print("==========================================================================================")
    ...
    for vol_th in thresholds:
        trades, equity = run_strategy_backtest(data_map, vol_spike_mult=vol_th)
        tot = len(trades)
    ...
    # Display sample trades log for 15x threshold
    sample_trades, _ = run_strategy_backtest(data_map, vol_spike_mult=15.0)
    print("="*90)
    print("📋 SAMPLE TRADES EXECUTED BY ACTUAL FIBVOL STRATEGY FILE (15x Spike | Entry 0.50 | SL 0.60 | 1:5 RR):")
    print("="*90)
    for tr in sample_trades[:20]:
        print(f"- {tr['symbol']:<10} | {tr['time_ist']} | {tr['spike_mult']:>4.1f}x | {tr['outcome']:<8} | PnL: {tr['pnl_r']:+4.1f}R (${tr['pnl_usd']:+6.2f}) | Bal: ${tr['wallet_usd']:>6.2f} | Hold: {tr['holding_mins']}m")
