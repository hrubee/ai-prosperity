#!/usr/bin/env python3
"""backtest_volume_start_short.py — Backtests the Mean Reversion Shorting strategy
using the LOW of the initial Volume Spike candle as the objective pump initiation start price.
"""
...
RISK_USD = 10.00
FEE_R = 0.05
TP_RETRACE = 0.70  # Champion 70% TP retracement

# Sweep different volume spikes
vol_spikes = [15.0, 20.0, 25.0, 30.0, 40.0]

print("==================================================================================")
print("📊 BACKTEST: INITIAL VOLUME SPIKE LOW AS START PRICE (JUNE 2026)")
print("==================================================================================")
...
for vol_threshold in vol_spikes:
    ...
    for sym, rows in raw_candles.items():
        ...
        while idx < len(bars_4h):
            ...
            if not in_watchlist:
                # Find initial volume spike (green candle + volume threshold)
                if (b['close'] > b['open']) and b['vol_mult'] >= vol_threshold:
                    in_watchlist = True
                    # Start Price is strictly the LOW of this volume spike candle
                    pump_start_px = b['low']
                    # Search for local peak high over next 8 days (48 candles)
                    ...
                    # Move index past the peak search
                ...
                if b['is_red'] and b['close'] < b['ema_9']:
                    ...
                    # Take Profit at 70% retracement of the pump height (referenced to spike low)
                    tp_px = pump_peak_px - TP_RETRACE * (pump_peak_px - pump_start_px)
