#!/usr/bin/env python3
"""Test effect of different concurrent position caps (5, 10, 20, 50, Unlimited)
with full real-world fees (0.10%) and slippage (0.10%).
"""
import sqlite3
import numpy as np
import pandas as pd
...
# Test 15m (10x spike, 1.5x ATR SL, 1:3 RR) across different concurrent position limits
tf_min = 15
spk_mult = 10.0
atr_mult = 1.5
rr = 3.0
...
print(f"Total Candidate Signals on 15m: {len(candidate_signals)}")

caps = [1, 3, 5, 10, 20, 30, 50, 999]

print(f"\n{'Max Concur':<12} | {'Trades':<8} | {'Win Rate':<10} | {'PF':<6} | {'Net Return (R)':<16} | {'$100 ROI':<12} | {'Max DD %'}")
print("-" * 80)

for max_concurrent in caps:
    active_positions = []
    executed_trades = []
    wallet_bal = 100.0
    peak_bal = 100.0
