#!/usr/bin/env python3
"""backtest_0.7_0.8_june_vps.py — Fast 1m granular backtest for FibVOL (0.7 Entry / 0.8 SL).
...
print(f"==================================================", flush=True)
print(f"📊 FIBVOL 1M GRANULAR BACKTEST: JUNE 2026 DATASET", flush=True)
print(f"   Database: {DB_PATH}", flush=True)
print(f"   Parameters: Entry Fib = 0.700 | SL Fib = 0.800 | RR = 1:5.0", flush=True)
print(f"==================================================", flush=True)
...
# Load 1m candles grouped by symbol directly via SQL for ultra speed
...
print(f"Found {len(symbols)} symbols. Processing 1m candle replays...", flush=True)

for i, sym in enumerate(symbols):
    if (i + 1) % 50 == 0 or (i + 1) == len(symbols):
        print(f"  [Progress] Processed {i+1}/{len(symbols)} symbols ({total_trades} trades filled so far...)", flush=True)
...
print("\n==================================================")
print("📊 1M GRANULAR BACKTEST RESULTS (0.7 ENTRY / 0.8 SL)")
print("==================================================")
