#!/usr/bin/env python3
"""backtest_realistic_fib_short.py — Realistic backtest of the Fib Short strategy
without lookahead bias. The peak is only confirmed after a red close below 9 EMA,
and limit orders are placed only if the price is still above the 23.6% retracement level.
"""
...
print("==================================================================================")
print("📊 REALISTIC BACKTEST: NO LOOKAHEAD BIAS | 23.6% LIMIT ENTRY | TP 78.6% (JUNE)")
print("==================================================================================")
...
for vol_threshold in vol_spikes:
    ...
    for sym, rows in raw_candles.items():
        ...
        while idx < len(bars_4h):
            ...
            if not in_watchlist:
                ...
                # Update running peak during watch phase
                if b['high'] > pump_peak_px:
                    pump_peak_px = b['high']
                    
                # Check for peak confirmation: first red close below 9 EMA
                if b['is_red'] and b['close'] < b['ema_9']:
                    ...
                    if risk > 0 and tp_px < entry_px:
                        # REALISTIC CHECK: If current price is already below our limit entry price,
                        # we cannot get filled at 23.6% retracement anymore!
                        if confirm_px < entry_px:
                            skipped_already_passed += 1
                        else:
                            # We can place the limit order at entry_px. Trace subsequent 1m candles.
    ...
    if trades:
        ...
        print(f"- Spike Mult: {int(vol_threshold):>2}x | Trades: {tot:<3} | Win Rate: {win_rate:.1f}% | Profit Factor: {pf:.2f} | Avg R: {avg_r:+.2f}R | Net Profit: ₹{net_inr:,.2f} | Skipped (Already Passed): {skipped_already_passed}")
    else:
        print(f"- Spike Mult: {int(vol_threshold)}x | No trades executed | Skipped (Already Passed): {skipped_already_passed}")
