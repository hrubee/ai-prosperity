...
trades = []
print(f"Running backtest for Multi-Candle Pullback Sweep (50/50 Scale Out) across 150 coins...")
...
for i, sym in enumerate(symbols):
    ...
    for idx in spikes:
        ...
        # Look forward up to 40 bars (10 hours) for a multi-candle pullback
        future_df = df.iloc[idx+1 : idx+41]
        
        for f_idx, f_row in future_df.iterrows():
            if f_idx < 3: continue # Need at least 3 candles of history
            
            # Ensure we are actually pulling back (price is below the spike high)
            if f_row['c'] >= spike_high:
                continue
                
            # The Multi-Candle Sweep Logic
            # We find the lowest low of the preceding 3 candles to define local structure.
            prev_3_low = df.iloc[f_idx - 3 : f_idx]['l'].min()
            
            # Trigger: Current candle sweeps the multi-candle low, gets bought up, and closes green.
            if f_row['l'] < prev_3_low and f_row['c'] > prev_3_low and f_row['c'] > f_row['o']:
...
summary = f"""
### 📈 Multi-Candle Pullback Backtest Results
**Timeframe:** 15m
**Signal:** 15x Volume Spike + Min 5% GREEN Pump Candle
**Trigger:** Multi-Candle Sweep (Current candle sweeps the lowest point of the previous 3 candles and closes green).
...
with open("scratch/bullish_multicandle_results.md", "w") as f:
    f.write(summary)
    f.write("\n\n#### Trade Log\n")
...
print("\nSaved to scratch/bullish_multicandle_results.md")
