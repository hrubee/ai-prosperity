#!/usr/bin/env python3
"""tools/backtest/test_fibema_vol_confirmed.py

Tests "fibema" with Volume Confirmation (Impulse bar has >= 2.0x, 3.0x, 5.0x volume surge):
- 21 EMA calculated on candle LOWs
- Long-only
- Deep Fib entry (0.618, 0.705, 0.786)
- SL below 21 EMA vs Below Swing Low
- RR 1:2 and 1:3
"""
import sqlite3
import numpy as np
...
for tf in [15, 60, 240]:
    ...
    for vol_mult in [1.0, 2.0, 3.0, 5.0]:
        setups = []
        for sym, m1_arr in sym_arrays.items():
            ...
            vol_avg = np.zeros(n)
            for vi in range(n):
                vol_avg[vi] = np.mean(vols[max(0, vi-20):vi]) if vi > 0 else vols[0]
            ...
            for i in range(21, n - 2):
                ...
                # Volume filter
                if vol_mult > 1.0 and (vols[i] / max(1.0, vol_avg[i])) < vol_mult:
                    continue
                ...
                setups.append({
                    ...
                    "start_idx": start_idx, "m1_h": m1_h, "m1_l": m1_l, "m1_c": m1_c, "tf_len": tf
                })
                
        for fib_entry in [0.500, 0.618, 0.705, 0.786]:
            for sl_name, sl_mode in [("Below EMA21", "below_ema"), ("Swing Low", "swing_low"), ("1.0x ATR", "atr_10")]:
                for rr in [1.5, 2.0, 2.5, 3.0]:
                    ...
                    pf = gw / gl
                    results.append({
                        "tf": tf_name, "vol_mult": vol_mult, "fib_entry": fib_entry,
                        "sl_type": sl_name, "rr": rr, "trades": n, "win_rate": wr, "pf": pf, "total_r": tot_r
                    })

results.sort(key=lambda x: x["total_r"], reverse=True)

print("\n" + "=" * 125)
print("🏆 TOP 25 FIBEMA CONFIGURATIONS (WITH VOLUME CONFIRMATION & 1M INTRABAR PRECISION)")
print("=" * 125)
print(f"{'TF':<5} | {'Vol Spike':<10} | {'Fib Entry':<10} | {'Stop Loss':<15} | {'RR Target':<10} | {'Trades':<8} | {'Win Rate':<10} | {'PF':<6} | {'Total Return (R)'}")
print("-" * 125)
for r in results[:25]:
    vol_str = f">={r['vol_mult']:.0f}x Vol" if r['vol_mult'] > 1.0 else "No Vol Filter"
    print(f"{r['tf'].upper():<5} | {vol_str:<10} | {r['fib_entry']:<10.3f} | {r['sl_type']:<15} | 1:{r['rr']:<8.1f} | {r['trades']:<8} | {r['win_rate']:>6.1f}%    | {r['pf']:<6.2f} | {r['total_r']:>+10.2f} R")
print("=" * 125)
