#!/usr/bin/env python3
"""audit_today_coindcx_fills.py — Audit direct CoinDCX exchange fills for TODAY ONLY (Aug 13, 2026 IST).
"""
import sys
import os
import json
...
def main():
    ...
    print("\n=== TODAY (AUG 13, 2026 IST) DIRECT EXECUTED FILLS FROM COINDCX SERVERS ===")
    all_fills = []
    ...
    # Cutoff for Aug 13, 2026 00:00:00 IST (1786560000000 ms)
    cutoff_ms = 1786560000000
    today_fills = [f for f in all_fills if float(f.get("timestamp") or 0) >= cutoff_ms]

    print(f"Total Direct Exchange Fills Today: {len(today_fills)}\n")
    ...
    print("-" * len(header))
    print(f"🎯 TODAY DIRECT EXCHANGE REALIZED NET PNL: ${total_net_usdt:+.2f} USD | Rs.{total_net_usdt * 86.0:+.2f} INR")
